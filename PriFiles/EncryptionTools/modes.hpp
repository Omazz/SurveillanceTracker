...
template <typename CipherType>
ByteBlock CFB_Mode<CipherType>::encrypt(const ByteBlock & src, ByteBlock & dst) const {
    std::vector<ByteBlock> blocks = split_blocks(src, CipherType::block_lenght);
    ByteBlock tmp;
    ...
    return ByteBlock(blocks[blocks.size() - 1].byte_ptr(), blocks[blocks.size() - 1].size());
}
